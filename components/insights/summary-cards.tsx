...
export function SummaryCards({ range }: SummaryCardsProps) {
  ...
  const householdAdherence =
    householdTotals.scheduled > 0 ? Math.round((householdTotals.completed / householdTotals.scheduled) * 100) : 0

  // Sort animals by adherence for leaderboard
  const sortedAnimals = [...complianceData].sort((a, b) => b.adherencePct - a.adherencePct)
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Household Streak</CardTitle>
          <TrendingUp className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{householdStreak} days</div>
          <p className="text-xs text-muted-foreground">No missed scheduled doses</p>
        </CardContent>
      </Card>

      {/* Overall Compliance */}
      <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => handleCardClick("type=all")}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card
        className="cursor-pointer hover:shadow-md transition-shadow"
        onClick={() => handleCardClick(`animalId=${sortedAnimals[0]?.animalId}`)}
      >
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <CardContent>
          {sortedAnimals[0] && (
            <div className="flex items-center gap-2">
              <AnimalAvatar animal={animals.find((a) => a.id === sortedAnimals[0].animalId)!} size="sm" />
              <div>
                <div className="font-bold">{sortedAnimals[0].animalName}</div>
                <div className="text-sm text-muted-foreground">{sortedAnimals[0].adherencePct}% adherence</div>
              </div>
            </div>
          )}
        </CardContent>
      ...
      >
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Missed doses this period</p>
        </CardContent>
      </Card>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={
                        animal.adherencePct >= 90 ? "default" : animal.adherencePct >= 80 ? "secondary" : "destructive"
                      }
                    >
                      {animal.adherencePct}%
                    </Badge>
