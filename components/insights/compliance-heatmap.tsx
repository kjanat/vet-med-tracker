...
export function ComplianceHeatmap({ range, onRangeChange }: ComplianceHeatmapProps) {
  ...
  const getBucketData = (dow: number, hour: number): HeatmapBucket | null => {
    return heatmapData.buckets.find((b) => b.dow === dow && b.hour === hour) || null
  }
  ...
  const getCellColor = (bucket: HeatmapBucket | null): string => {
    ...
    if (problemRate >= 15) return `bg-orange-${Math.round(intensity * 400) + 100}`
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Medication timing patterns by day of week and hour</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Select value={selectedRegimenId} onValueChange={setSelectedRegimenId}>
              <SelectTrigger className="w-[180px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                ...
                <div className="p-4 space-y-4">
                  <div className="space-y-2">
                    ...
                    <CalendarComponent
                      mode="single"
                      selected={range.from}
                      onSelect={(date) => date && onRangeChange({ ...range, from: date })}
                      initialFocus
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <CalendarComponent
                      mode="single"
                      selected={range.to}
                      onSelect={(date) => date && onRangeChange({ ...range, to: date })}
                    />
                  </div>
                </div>
              </PopoverContent>
          ...
          <div className="space-y-2">
            ...
            {dayNames.map((day, dow) => (
              <div key={dow} className="grid grid-cols-25 gap-1">
                <div className="text-xs font-medium text-muted-foreground py-1">{day}</div>
                {hours.map((hour) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        getCellColor(bucket),
                        bucket && bucket.count > 0 ? "hover:ring-2 hover:ring-primary" : "",
                      )}
                      onClick={() => bucket && bucket.count > 0 && handleCellClick(bucket)}
                      title={
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>{selectedCell && `${dayNames[selectedCell.dow]} ${selectedCell.hour}:00`}</SheetTitle>
          </SheetHeader>
          ...
          {selectedCell && (
            <div className="mt-6 space-y-4">
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-orange-600">{selectedCell.latePct}%</div>
                  <div className="text-sm text-muted-foreground">Late</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-red-600">{selectedCell.missedPct}%</div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="p-3 border rounded-lg">
                    ...
                    <div className="text-sm text-muted-foreground">Recorded at 8:15 AM • 15 min late</div>
                    <Badge variant="secondary" className="mt-1">
                      Late
                    </Badge>
                  </div>
                  <div className="p-3 border rounded-lg">
                    <div className="font-medium">Whiskers - Insulin 2 units</div>
                    <div className="text-sm text-muted-foreground">Recorded at 8:02 AM • On time</div>
