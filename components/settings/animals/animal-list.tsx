...
export function AnimalList() {
  ...
  const filteredAnimals = mockAnimals.filter((animal) => animal.name.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  const handleSave = async (data: Partial<Animal>) => {
    ...
    window.dispatchEvent(
      new CustomEvent(editingAnimal ? "settings_animals_update" : "settings_animals_create", {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage animal profiles and medical information</p>
        </div>
          ...
          <Card key={animal.id} className="hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                {animal.weightKg && (
                  <div>
                    <span className="font-medium">Weight:</span> {animal.weightKg}kg
                  </div>
                )}
                {animal.dob && (
                  <div>
                    <span className="font-medium">Age:</span>{" "}
                    {Math.floor((Date.now() - animal.dob.getTime()) / (365.25 * 24 * 60 * 60 * 1000))} years
                  </div>
                )}
                <div>
                  <span className="font-medium">Timezone:</span> {animal.timezone.split("/")[1]?.replace("_", " ")}
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <Button variant="outline" size="sm" onClick={() => handleEdit(animal)} className="flex-1">
                  Edit
                </Button>
                <Button variant="outline" size="sm" onClick={() => handleEmergencyCard(animal.id)} className="gap-1">
      ...
      {/* Animal Form */}
      <AnimalForm animal={editingAnimal} open={isFormOpen} onOpenChange={setIsFormOpen} onSave={handleSave} />
    </div>
  )
}
