...
export function HistoryCalendar({ month, counts, records, onSelectDay, onMonthChange }: HistoryCalendarProps) {
  ...
  const getDayRecords = (day: Date) => records.filter((r) => isSameDay(r.recordedAt, day))
                ...
                >
                  ...
                  {dayCount && (
                    <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2">
                      <div className={cn("w-2 h-2 rounded-full", getStatusColor(dayCount))} />
                    </div>
                  )}
                </button>
              </SheetTrigger>
                  ...
                  <div className="mt-6 space-y-4">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">{dayCount.onTime}</div>
                        <div className="text-xs text-muted-foreground">On-time</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-yellow-600">{dayCount.late}</div>
                        <div className="text-xs text-muted-foreground">Late</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-red-600">{dayCount.missed}</div>
                        <div className="text-xs text-muted-foreground">Missed</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">{dayCount.prn}</div>
                        ...
                        <div key={record.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              {format(record.recordedAt, "h:mm a")} • {record.caregiverName}
                            </div>
                          </div>
                          <Badge
                            variant={
