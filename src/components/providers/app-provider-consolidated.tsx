...
import { useHouseholdActions } from "./use-household-actions.ts";
import { useStackMetadataPreferences } from "./use-stack-metadata-preferences.ts";
...
export type VetMedPreferences =
  (typeof vetmedUsers.$inferSelect)["preferences"];
...
// Import defaults from schema - these use null instead of empty strings
// which is semantically correct for optional fields
const defaultVetMedPreferences: VetMedPreferences = defaultUserPreferences;
    ...
    async (updates: Partial<VetMedPreferences>) => {
      ...
      try {
        await persistPreferences({
          vetMedPreferences: {
            defaultAnimalId: newPreferences.defaultAnimalId ?? undefined,
            defaultHouseholdId:
            ...
            emergencyContactName:
              newPreferences.emergencyContactName ?? undefined,
            emergencyContactPhone:
              newPreferences.emergencyContactPhone ?? undefined,
            notificationPreferences: newPreferences.notificationPreferences,
            preferredPhoneNumber:
              newPreferences.preferredPhoneNumber ?? undefined,
          },
        });
      } catch (error) {
