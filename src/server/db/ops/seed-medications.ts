...
import { vetmedMedicationCatalog } from "@/db/schema";
...
export async function seedMedicationCatalog(
  ...
  const countResult = await db.execute(
    sql`SELECT COUNT(*)::int AS count FROM ${vetmedMedicationCatalog}`,
  );
  ...
  for (const medication of commonVeterinaryMedications) {
    const existing = await db
      .select()
      .from(vetmedMedicationCatalog)
      .where(eq(vetmedMedicationCatalog.genericName, medication.genericName))
      .limit(1);
    ...
    await db.insert(vetmedMedicationCatalog).values(payload);
    inserted += 1;
    log(
