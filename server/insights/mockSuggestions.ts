...
export function mockSuggestions(householdId: string): Suggestion[] {
	const suggestions: Suggestion[] = [
		...
		},
		{
			...
			rationale:
				"Two overlapping administrations detected last week (risk of double-dosing)",
			action: {
			...
			priority: "high",
			estimatedImpact:
				"Prevents accidental double-dosing of high-risk medication",
		},
		{
			...
			priority: "high",
			estimatedImpact:
				"Critical for diabetic management - could prevent emergencies",
		},
		{
			...
			rationale:
				"Consistently late by 45+ minutes (family dinner time conflict)",
			action: {
	...
	return suggestions
		.slice(startIndex)
		.concat(suggestions.slice(0, startIndex))
		.slice(0, 3);
}
