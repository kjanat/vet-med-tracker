import { pgTable, text, timestamp, uuid, integer, boolean, date, decimal, index, pgEnum } from 'drizzle-orm/pg-core';
...
export const routeEnum = pgEnum('vetmed_route', [
  'ORAL', 'SC', 'IM', 'IV', 'TOPICAL', 'OTIC', 'OPHTHALMIC', 'INHALED', 'RECTAL', 'OTHER'
]);

export const formEnum = pgEnum('vetmed_form', [
  'TABLET', 'CAPSULE', 'LIQUID', 'INJECTION', 'CREAM', 'OINTMENT', 'DROPS', 
  'SPRAY', 'POWDER', 'PATCH', 'OTHER'
]);

export const storageEnum = pgEnum('vetmed_storage', ['ROOM', 'FRIDGE', 'FREEZER', 'CONTROLLED']);

// Medication Catalog (shared reference data)
export const medicationCatalog = pgTable('vetmed_medication_catalog', {
...
export const inventoryItems = pgTable('vetmed_inventory_items', {
  id: uuid('id').defaultRandom().primaryKey(),
  householdId: uuid('household_id').notNull().references(() => households.id, { onDelete: 'cascade' }),
  medicationId: uuid('medication_id').notNull().references(() => medicationCatalog.id),
  assignedAnimalId: uuid('assigned_animal_id').references(() => animals.id, { onDelete: 'set null' }),
