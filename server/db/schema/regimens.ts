import { pgTable, text, timestamp, uuid, boolean, integer, date, time, index, pgEnum } from 'drizzle-orm/pg-core';
...
// Enums
export const scheduleTypeEnum = pgEnum('vetmed_schedule_type', ['FIXED', 'PRN', 'INTERVAL', 'TAPER']);
export const adminStatusEnum = pgEnum('vetmed_admin_status', ['ON_TIME', 'LATE', 'VERY_LATE', 'MISSED', 'PRN']);

// Regimens (medication plans)
export const regimens = pgTable('vetmed_regimens', {
  id: uuid('id').defaultRandom().primaryKey(),
  animalId: uuid('animal_id').notNull().references(() => animals.id, { onDelete: 'cascade' }),
  medicationId: uuid('medication_id').notNull().references(() => medicationCatalog.id),
...
export const administrations = pgTable('vetmed_administrations', {
  id: uuid('id').defaultRandom().primaryKey(),
  
  // Core relationships
  regimenId: uuid('regimen_id').notNull().references(() => regimens.id),
  ...
  idempotencyKeyIdx: index('admin_idempotency_key_idx').on(table.idempotencyKey),
}));
...
export const administrationsRelations = relations(administrations, ({ one }) => ({
