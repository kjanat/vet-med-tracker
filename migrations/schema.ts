...
export const vetmedCosignStatus = pgEnum("vetmed_cosign_status", ['pending', 'approved', 'rejected', 'expired'])
...
export const vetmedSuggestionStatus = pgEnum("vetmed_suggestion_status", ['pending', 'applied', 'reverted', 'dismissed'])
...
export const vetmedSuggestions = pgTable("vetmed_suggestions", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	householdId: uuid("household_id").notNull(),
	type: text().notNull(),
	...
	index("suggestions_created_at_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("suggestions_household_id_idx").using("btree", table.householdId.asc().nullsLast().op("uuid_ops")),
	index("suggestions_status_idx").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("suggestions_type_idx").using("btree", table.type.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.householdId],
			foreignColumns: [vetmedHouseholds.id],
...
export const vetmedPushSubscriptions = pgTable("vetmed_push_subscriptions", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("push_subscription_active_idx").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("push_subscription_endpoint_idx").using("btree", table.endpoint.asc().nullsLast().op("text_ops")),
	index("push_subscription_user_idx").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
...
export const vetmedCosignRequests = pgTable("vetmed_cosign_requests", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("cosign_requests_administration_id_idx").using("btree", table.administrationId.asc().nullsLast().op("uuid_ops")),
	index("cosign_requests_cosigner_id_idx").using("btree", table.cosignerId.asc().nullsLast().op("uuid_ops")),
	index("cosign_requests_expires_at_idx").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("cosign_requests_household_id_idx").using("btree", table.householdId.asc().nullsLast().op("uuid_ops")),
	index("cosign_requests_requester_id_idx").using("btree", table.requesterId.asc().nullsLast().op("uuid_ops")),
	index("cosign_requests_status_idx").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	foreignKey({
	...
	foreignKey({
			...
			name: "vetmed_cosign_requests_administration_id_vetmed_administrations"
		}).onDelete("cascade"),
	unique("vetmed_cosign_requests_administration_id_unique").on(table.administrationId),
]);
...
export const vetmedInventoryItems = pgTable("vetmed_inventory_items", {
	...
	isCustomMedication: boolean("is_custom_medication").default(false).notNull(),
}, (table) => [
...
export const vetmedNotifications = pgTable("vetmed_notifications", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	userId: uuid("user_id").notNull(),
	householdId: uuid("household_id").notNull(),
	type: text().notNull(),
	...
	index("notification_household_idx").using("btree", table.householdId.asc().nullsLast().op("uuid_ops")),
	index("notification_type_idx").using("btree", table.type.asc().nullsLast().op("text_ops")),
	index("notification_user_read_idx").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.read.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [vetmedUsers.id],
...
export const vetmedUsers = pgTable("vetmed_users", {
	...
	profileVisibility: jsonb("profile_visibility").default({"bio":true,"name":true,"email":false,"location":true}),
...
export const vetmedMedicationCatalog = pgTable("vetmed_medication_catalog", {
	...
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	dosageMinMgKg: numeric("dosage_min_mg_kg", { precision: 10, scale:  4 }),
	dosageMaxMgKg: numeric("dosage_max_mg_kg", { precision: 10, scale:  4 }),
	dosageTypicalMgKg: numeric("dosage_typical_mg_kg", { precision: 10, scale:  4 }),
	...
	index("med_catalog_dosage_range_idx").using("btree", table.dosageMinMgKg.asc().nullsLast().op("numeric_ops"), table.dosageMaxMgKg.asc().nullsLast().op("numeric_ops")),
