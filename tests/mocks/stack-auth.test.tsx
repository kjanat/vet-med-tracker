...
import { renderWithAuth, AuthTestScenarios } from "../helpers/stack-auth-test-utils";
...
describe("Stack Auth Mock System", () => {
  ...
  describe("Mock Stack Server App", () => {
    ...
    it("should handle redirect option", async () => {
      ...
      await expect(serverApp.getUser({ or: "redirect" }))
        .rejects
        .toThrow("REDIRECT_TO_SIGNIN");
    });

    it("should handle throw option", async () => {
      const serverApp = createMockStackServerApp(null);

      await expect(serverApp.getUser({ or: "throw" }))
        .rejects
  ...
  describe("StackAuthTestUtils", () => {
    ...
    it("should execute callback with authenticated user", () => {
      ...
      const result = StackAuthTestUtils.withAuthenticatedUser(TEST_USERS.CAREGIVER, () => {
  ...
  describe("Test Helper Integration", () => {
    it("should render with authenticated user", () => {
      ...
      expect(screen.getByTestId("user-display-name")).toHaveTextContent("Test Owner");
      expect(screen.getByTestId("user-email")).toHaveTextContent("owner@vetmed.test");
    });
