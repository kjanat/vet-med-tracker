...
export interface MockStackServerApp {
  getUser: (options?: { or?: "redirect" | "throw" }) => Promise<MockStackUser | null>;
  signInUrl: string;
  signUpUrl: string;
  signOutUrl: string;
...
export const createMockUser = (overrides: Partial<MockStackUser> = {}): MockStackUser => {
...
export const createMockStackServerApp = (defaultUser: MockStackUser | null = null): MockStackServerApp => ({
...
export const MockStackProvider = ({ children }: { children: React.ReactNode }) => {
...
export class StackAuthTestUtils {
  ...
  static initialize() {
    this.mockStackServerApp = createMockStackServerApp(this.mockUser);
  }
  ...
  static setMockUser(user: MockStackUser | null) {
    this.mockUser = user;
    this.mockStackServerApp = createMockStackServerApp(user);
  }
  ...
  static getMockStackServerApp(): MockStackServerApp {
    return this.mockStackServerApp;
  }
  ...
  static reset() {
    this.mockUser = null;
    this.mockStackServerApp = createMockStackServerApp(null);
    vi.clearAllMocks();
  }
   ...
   */
  static withAuthenticatedUser<T>(user: MockStackUser, callback: (user: MockStackUser) => T): T {
    const originalUser = this.mockUser;
    this.setMockUser(user);
    try {
      return callback(user);
    } finally {
      this.setMockUser(originalUser);
    }
  }
...
export class StackAuthPlaywrightHelpers {
   ...
   */
  static async mockStackAuth(page: any, user: MockStackUser | null = TEST_USERS.OWNER) {
    ...
    await page.addInitScript((userData) => {
      if (userData) {
        localStorage.setItem("stack-auth-session", JSON.stringify({
          user: userData,
  ...
  static async signIn(page: any, user: MockStackUser = TEST_USERS.OWNER) {
    await this.mockStackAuth(page, user);
    ...
    await page.evaluate((userData) => {
      ...
      window.dispatchEvent(new CustomEvent("stack-auth-sign-in", {
  ...
  static async signOut(page: any) {
    await this.mockStackAuth(page, null);
...
export const ClerkCompatibilityLayer = {
  ...
  createMockClerkUser: (userData: any) => createMockUser({
    displayName: `${userData.firstName} ${userData.lastName}`,
    primaryEmail: userData.emailAddresses?.[0]?.emailAddress || userData.email,
    clientMetadata: {
      onboardingComplete: userData.publicMetadata?.onboardingComplete || false,
