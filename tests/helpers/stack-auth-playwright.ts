...
import { TEST_USERS, type MockStackUser } from "../mocks/stack-auth";

export class StackAuthPlaywrightHelpers {
  /**
   * Mock Stack Auth for Playwright tests by intercepting API calls
   */
  static async mockStackAuth(page: Page, user: MockStackUser | null = TEST_USERS.OWNER) {
  ...
  static async signIn(page: Page, user: MockStackUser = TEST_USERS.OWNER) {
    ...
    await page.evaluate((userData) => {
      ...
      window.dispatchEvent(new CustomEvent("stack-auth-state-change", {
    ...
    await page.waitForURL(/\/(dashboard|profile|onboarding|manage)/, { timeout: 10000 })
      .catch(async () => {
  ...
  static async signOut(page: Page) {
    ...
    const userMenuButton = page.locator('[data-testid="user-menu"], [aria-label*="user menu"]').first();
  ...
  static async expectAuthenticated(page: Page, user?: MockStackUser) {
    ...
    await expect(page).not.toHaveURL(/\/(sign-in|sign-up|handler\/sign-in|handler\/sign-up)/);
  }
  ...
  static createAuthFlows(page: Page) {
    return {
       ...
       */
      asOwner: async () => {
        await this.signIn(page, TEST_USERS.OWNER);
        await this.expectAuthenticated(page, TEST_USERS.OWNER);
      },
       ...
       */
      asCaregiver: async () => {
        await this.signIn(page, TEST_USERS.CAREGIVER);
        await this.expectAuthenticated(page, TEST_USERS.CAREGIVER);
      },
       ...
       */
      asVet: async () => {
        await this.signIn(page, TEST_USERS.VET_READONLY);
        await this.expectAuthenticated(page, TEST_USERS.VET_READONLY);
      },
      ...
      asGuest: async () => {
        ...
        await this.expectUnauthenticated(page);
      },
      ...
      asCustomUser: async (user: MockStackUser) => {
        ...
        await this.expectAuthenticated(page, user);
      },
    };
  }
   ...
   */
  static async testProtectedRoute(page: Page, route: string, expectedRedirect: string = "/handler/sign-in") {
   ...
   */
  static async testAuthenticatedRoute(page: Page, route: string, user: MockStackUser = TEST_USERS.OWNER) {
    // Sign in user
    await this.signIn(page, user);
