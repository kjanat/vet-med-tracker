...
export const AuthTestScenarios = {
  ...
  withCaregiver: (callback: (user: MockStackUser) => void | Promise<void>) => {
    return StackAuthTestUtils.withAuthenticatedUser(
 ...
 */
export const simulateSignIn = async (
  user: MockStackUser = TEST_USERS.OWNER,
) => {
  StackAuthTestUtils.setMockUser(user);
 ...
 */
export const createAuthenticatedTRPCContext = (
  user: MockStackUser = TEST_USERS.OWNER,
) => {
  StackAuthTestUtils.setMockUser(user);
  ...
  convertClerkUser: (clerkUser: any): MockStackUser => {
    return createMockUser({
      ...
      primaryEmailVerified:
        clerkUser.emailAddresses?.[0]?.verification?.status === "complete",
      clientMetadata: {
        onboardingComplete:
          clerkUser.publicMetadata?.onboardingComplete || false,
