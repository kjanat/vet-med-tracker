 ...
 */
export function generateTestEmail(baseEmail: string = "test@example.com"): string {
...
export const ClerkTestHelpers = {
  ...
  async signInWithTestUser(
    ...
    await page.fill('input[name="code"]', CLERK_TEST_CONSTANTS.VERIFICATION_CODE);
    await page.click('button[type="submit"]');
...
export const ClerkMockHelpers = {
   ...
   */
  createMockClerkUser(user = TEST_USERS.OWNER) {
    return {
      id: `user_${Date.now()}`,
   ...
   */
  createTestSession(user = TEST_USERS.OWNER) {
    return {
