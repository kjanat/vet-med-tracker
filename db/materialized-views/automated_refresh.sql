...
CREATE
OR REPLACE FUNCTION trigger_compliance_refresh()
RETURNS TRIGGER AS $$
DECLARE
...
IF
last_refresh IS NULL OR last_refresh < NOW() - INTERVAL '5 minutes' THEN
...
CREATE
OR REPLACE FUNCTION trigger_inventory_refresh()
RETURNS TRIGGER AS $$
DECLARE
last_refresh TIMESTAMP WITH TIME ZONE;
BEGIN
...
INTO last_refresh
FROM mv_refresh_log
WHERE view_name = 'mv_inventory_consumption'
  AND status = 'COMPLETED';

IF
last_refresh IS NULL OR last_refresh < NOW() - INTERVAL '15 minutes' THEN
...
CREATE
OR REPLACE VIEW mv_refresh_schedule AS
SELECT jobname,
       schedule,
       command,
       nodename,
...
WHERE jobname LIKE '%refresh%'
   OR jobname LIKE '%materialized%'
ORDER BY jobname;

-- View to monitor refresh performance and health
CREATE
OR REPLACE VIEW mv_refresh_health AS
...
CREATE
OR REPLACE FUNCTION manual_refresh_all()
...
FOR result IN
SELECT *
FROM refresh_all_materialized_views() LOOP RETURN QUERY
SELECT format('%s: %s (%s ms, %s rows)',
              result.view_name,
              result.status,
