...
BEGIN
    RAISE
NOTICE 'Starting materialized views deployment at %', NOW();
END $$;

-- Step 1: Create refresh logging table
RAISE
NOTICE 'Step 1: Creating refresh log table...';
\i
...
BEGIN
    ...
    FOREACH
view_name IN ARRAY expected_views LOOP
SELECT COUNT(*)
INTO view_count
FROM pg_matviews
...
BEGIN
    ...
    FOREACH
func_name IN ARRAY expected_functions LOOP
SELECT COUNT(*)
INTO func_count
FROM pg_proc p
...
END LOOP;
    ...
    -- Report results
    IF
array_length(missing_functions, 1) IS NULL THEN
...
RAISE
NOTICE 'Step 5: Testing initial refresh...';
...
BEGIN
    RAISE
NOTICE 'Running initial refresh of all materialized views...';

FOR result IN
SELECT *
FROM refresh_all_materialized_views() LOOP RAISE NOTICE 'View: %, Status: %, Duration: %ms, Rows: %',
            result.view_name, result.status, result.refresh_duration_ms, result.rows_count;

IF
result.status != 'COMPLETED' THEN
...
BEGIN
    RAISE
NOTICE '=== DEPLOYMENT SUMMARY ===';
    RAISE
NOTICE 'Deployment completed at: %', NOW();
    
    -- Show view sizes
    RAISE
NOTICE 'Materialized View Sizes:';
...
total_rows
:= total_rows + view_sizes.rows;
END LOOP;
    
    RAISE
NOTICE 'Total materialized data: % rows', total_rows;
