...
CREATE TABLE IF NOT EXISTS mv_refresh_log (
  ...
  error_message TEXT,
  refresh_type VARCHAR(20) DEFAULT 'CONCURRENT' -- CONCURRENT, FULL
);
...
BEGIN
  ...
  VALUES ('mv_compliance_stats', start_time, 'CONCURRENT')
  RETURNING id INTO log_id;
  
  BEGIN
    -- Refresh the materialized view concurrently
    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_compliance_stats;
    
    -- Get row count
    SELECT COUNT(*) INTO row_count FROM mv_compliance_stats;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Failed to refresh mv_compliance_stats: %', error_msg;
  END;
END;
$$ LANGUAGE plpgsql;

-- Function to refresh medication usage with monitoring
CREATE OR REPLACE FUNCTION refresh_medication_usage()
RETURNS TABLE(
  view_name TEXT,
  refresh_duration_ms INTEGER,
  rows_count BIGINT,
...
BEGIN
  ...
  INSERT INTO mv_refresh_log (view_name, refresh_started_at, refresh_type)
  VALUES ('mv_medication_usage', start_time, 'CONCURRENT')
  RETURNING id INTO log_id;
  
  BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_medication_usage;
    
    SELECT COUNT(*) INTO row_count FROM mv_medication_usage;
    end_time := NOW();
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Failed to refresh mv_medication_usage: %', error_msg;
  END;
END;
$$ LANGUAGE plpgsql;

-- Function to refresh inventory consumption with monitoring
CREATE OR REPLACE FUNCTION refresh_inventory_consumption()
RETURNS TABLE(
  view_name TEXT,
  refresh_duration_ms INTEGER,
  rows_count BIGINT,
...
BEGIN
  ...
  INSERT INTO mv_refresh_log (view_name, refresh_started_at, refresh_type)
  VALUES ('mv_inventory_consumption', start_time, 'CONCURRENT')
  RETURNING id INTO log_id;
  
  BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_inventory_consumption;
    
    SELECT COUNT(*) INTO row_count FROM mv_inventory_consumption;
    end_time := NOW();
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Failed to refresh mv_inventory_consumption: %', error_msg;
  END;
END;
$$ LANGUAGE plpgsql;

-- Function to refresh animal health trends with monitoring
CREATE OR REPLACE FUNCTION refresh_animal_health_trends()
RETURNS TABLE(
  view_name TEXT,
  refresh_duration_ms INTEGER,
  rows_count BIGINT,
...
BEGIN
  ...
  INSERT INTO mv_refresh_log (view_name, refresh_started_at, refresh_type)
  VALUES ('mv_animal_health_trends', start_time, 'CONCURRENT')
  RETURNING id INTO log_id;
  
  BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_animal_health_trends;
    
    SELECT COUNT(*) INTO row_count FROM mv_animal_health_trends;
    end_time := NOW();
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Failed to refresh mv_animal_health_trends: %', error_msg;
  END;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION get_mv_refresh_status()
RETURNS TABLE(
  view_name TEXT,
...
CREATE OR REPLACE FUNCTION cleanup_mv_refresh_logs()
RETURNS INTEGER AS $$
DECLARE
