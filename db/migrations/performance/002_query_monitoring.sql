...
CREATE OR REPLACE FUNCTION auto_maintenance_check()
RETURNS TEXT AS $$
DECLARE
    rec RECORD;
    maintenance_log TEXT := '';
    bloat_threshold NUMERIC := 25.0; -- 25% bloat threshold
BEGIN
    -- Check for tables with high bloat
    FOR rec IN 
        SELECT tablename, bloat_percentage 
        FROM v_table_bloat 
        WHERE bloat_percentage > bloat_threshold
    LOOP
        ...
        maintenance_log := maintenance_log || 'RECOMMEND: REINDEX TABLE ' || rec.tablename || chr(10);
    END LOOP;
    ...
    LOOP
        ...
        maintenance_log := maintenance_log || 'RECOMMEND: Consider dropping if truly unused' || chr(10);
    END LOOP;
...
BEGIN
    SELECT json_build_object(
        ...
        'index_usage', (SELECT json_agg(row_to_json(i)) FROM v_index_usage i WHERE usage_category = 'HIGH_USAGE'),
