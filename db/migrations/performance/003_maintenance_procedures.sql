...
CREATE OR REPLACE FUNCTION perform_intelligent_maintenance()
RETURNS TABLE (
    table_name TEXT,
...
BEGIN
    ...
    LOOP
        -- Get current size
        SELECT pg_total_relation_size(rec.tablename) INTO before_size;
        
        -- Determine maintenance action needed
        IF rec.bloat_pct > bloat_threshold OR rec.n_dead_tup > 1000 THEN
            -- High bloat - perform VACUUM FULL (use with caution in production)
            EXECUTE format('VACUUM ANALYZE %I', rec.tablename);
            
            SELECT pg_total_relation_size(rec.tablename) INTO after_size;
        ...
        ELSIF rec.last_analyze IS NULL OR rec.last_analyze < NOW() - INTERVAL '7 days' THEN
...
CREATE OR REPLACE FUNCTION rebuild_inefficient_indexes()
RETURNS TABLE (
    index_name TEXT,
    action TEXT,
    size_before TEXT,
...
BEGIN
    ...
    LOOP
        ...
        IF idx_rec.idx_scan = 0 AND NOT idx_rec.indexname LIKE '%_pkey' THEN
            ...
            RETURN QUERY SELECT 
                ...
                '0 bytes'::TEXT,
                format('Unused index consuming %s', pg_size_pretty(size_before));
        
        ELSIF idx_rec.idx_tup_read > 0 AND idx_rec.idx_tup_fetch::float / idx_rec.idx_tup_read < 0.1 THEN
            -- Low fetch ratio indicates potential bloat
            EXECUTE format('REINDEX INDEX %I', idx_rec.indexname);
            
            size_after := pg_relation_size(idx_rec.indexname);
...
BEGIN
    RETURN QUERY
    SELECT 
        encode(sha256(pss.query::bytea), 'hex') as query_hash,
...
DECLARE
    ...
    index_results JSON;
    slow_queries JSON;
    blocking_queries JSON;
BEGIN
    -- Perform table maintenance
    SELECT json_agg(row_to_json(t)) INTO maintenance_results
    FROM perform_intelligent_maintenance() t;
    
    -- Rebuild inefficient indexes
    SELECT json_agg(row_to_json(i)) INTO index_results
    FROM rebuild_inefficient_indexes() i;
    
    -- Identify slow queries
    SELECT json_agg(row_to_json(s)) INTO slow_queries
    FROM monitor_slow_queries(100) s;
    
    -- Check for blocking queries
    SELECT json_agg(row_to_json(b)) INTO blocking_queries
...
BEGIN
    ...
    IF cache_hit_ratio < 90 THEN
        RETURN QUERY SELECT 
            ...
            'WARNING'::TEXT,
            'Database cache hit ratio is below optimal threshold'::TEXT,
            cache_hit_ratio::TEXT || '%',
            '90%'::TEXT;
    END IF;
    ...
    IF active_connections > 50 THEN
        RETURN QUERY SELECT 
            ...
            'WARNING'::TEXT,
            'High number of active database connections'::TEXT,
    ...
    IF long_running_queries > 0 THEN
        RETURN QUERY SELECT 
            ...
            'CRITICAL'::TEXT,
            'Long-running queries detected (>5 minutes)'::TEXT,
            long_running_queries::TEXT,
            '0'::TEXT;
    END IF;
...
DECLARE
    ...
    operation_status TEXT := CASE WHEN error_msg IS NULL THEN 'COMPLETED' ELSE 'FAILED' END;
BEGIN
    INSERT INTO vetmed_maintenance_log (
        id, maintenance_type, completed_at, status, report, error_message
    ) VALUES (
        log_id, maintenance_type, NOW(), operation_status, maintenance_report, error_msg
    );
