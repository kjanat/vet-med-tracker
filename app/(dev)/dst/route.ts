...
function expandFixedTimes(times: string[], dateISO: string, timezone: string): Date[] {
  const slots: Date[] = []

  for (const timeLocal of times) {
    const dt = DateTime.fromISO(`${dateISO}T${timeLocal}:00`, { zone: timezone })
...
export async function GET() {
  try {
    ...
    const beforeDST = slots.filter((s) => DateTime.fromISO(s.timeUTC) < dstTransition)
    const afterDST = slots.filter((s) => DateTime.fromISO(s.timeUTC) >= dstTransition)
