...
import { HistoryList, type AdministrationRecord } from "@/components/history/history-list"
...
export default function HistoryPage() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    filtered = filtered.filter((r) => {
      const recordDate = new Date(r.recordedAt.getFullYear(), r.recordedAt.getMonth(), r.recordedAt.getDate())
  ...
  const groupedRecords = useMemo(() => {
    ...
    return Array.from(groups.entries())
      .map(([dateStr, records]) => ({
        ...
        records: records.sort((a, b) => b.recordedAt.getTime() - a.recordedAt.getTime()),
      }))
  ...
  const dailyCounts = useMemo(() => {
    ...
    const monthRecords = filteredRecords.filter((r) => r.recordedAt >= monthStart && r.recordedAt <= monthEnd)

    const counts = new Map<string, { total: number; onTime: number; late: number; missed: number; prn: number }>()
