...
export default function RecordPage() {
  ...
  const [selectedRegimen, setSelectedRegimen] = useState<DueRegimen | null>(null)
  const [selectedAnimalId, setSelectedAnimalId] = useState<string | null>(null)
  const [inventorySourceId, setInventorySourceId] = useState<string | null>(null)
  ...
  const filteredRegimens = selectedAnimalId ? dueRegimens.filter((r) => r.animalId === selectedAnimalId) : dueRegimens
      ...
      <div className="max-w-md mx-auto space-y-6">
        <div className="text-center space-y-4">
          <div className="h-16 w-16 bg-green-100 rounded-full flex items-center justify-center mx-auto">
            <div className="h-8 w-8 bg-green-500 rounded-full flex items-center justify-center">✓</div>
          </div>

          <div>
            <h1 className="text-2xl font-bold text-green-700">Recorded Successfully</h1>
  ...
  if (step === "confirm" && selectedRegimen) {
    ...
    const relevantSources = inventorySources.filter((s) =>
      s.name.toLowerCase().includes(selectedRegimen.medicationName.toLowerCase()),
    )
            ...
            <CardTitle className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm font-normal text-muted-foreground">
                  {selectedRegimen.strength} • {selectedRegimen.route} • {selectedRegimen.form}
                </div>
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {selectedRegimen.isHighRisk && (
              <div className="flex items-center space-x-2 p-4 bg-orange-50 rounded-lg border border-orange-200">
                <Checkbox id="cosign" checked={requiresCoSign} onCheckedChange={setRequiresCoSign} />
            ...
            <MedConfirmButton
              ...
              disabled={
                isSubmitting ||
                (relevantSources.some((s) => s.id === inventorySourceId && (s.isExpired || s.isWrongMed)) &&
                  !allowOverride)
              }
        ...
        {groupedRegimens.due.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Medications that are due or overdue</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {groupedRegimens.due.map((regimen) => (
                <RegimenCard key={regimen.id} regimen={regimen} onSelect={handleRegimenSelect} />
              ))}
            </CardContent>
          </Card>
        )}
        ...
        {groupedRegimens.later.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upcoming medications scheduled for today</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
        ...
        {groupedRegimens.prn.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Medications that can be given as needed</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
...
function RegimenCard({ regimen, onSelect }: { regimen: DueRegimen; onSelect: (regimen: DueRegimen) => void }) {
  const { animals } = useApp()
  const animal = animals.find((a) => a.id === regimen.animalId)

  const isOverdue = regimen.targetTime && regimen.targetTime < new Date()
  const timeDisplay = regimen.targetTime ? formatTimeLocal(regimen.targetTime, "America/New_York") : "As needed"
