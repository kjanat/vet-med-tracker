...
class PerformanceValidator {
  ...
  private async ensureTestData() {
    ...
    const existingAnimals = await db
      ...
      .from(animals)
      .where(sql`household_id
            =
            ${this.testHouseholdId}`)
      .limit(10);
    ...
    const testRegimens = await db
      ...
      .innerJoin(
        animals,
        sql`${regimens.animalId}
            =
            ${animals.id}`,
      )
      .where(sql`${animals.householdId}
            =
            ${this.testHouseholdId}`)
      .limit(10);
  ...
  private async runSingleBenchmark(
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        if (i === 0) {
          try {
            const explainQuery = sql`EXPLAIN
  ...
  private buildDueMedicationsQuery() {
    return sql`
            WITH active_regimens AS (SELECT r.id,
                                            r.schedule_type,
                                            r.times_local,
                                            r.dose,
                                            an.name as animal_name,
                                                     ...
                                                     CASE
                                                         WHEN EXISTS (SELECT 1
                                                                      ...
                                                                      WHERE ABS(EXTRACT(EPOCH FROM (
                                                                          (CURRENT_TIMESTAMP AT TIME ZONE ar.timezone)
                                                                                        ::time - time_element
                                                                                        ::time ))) <= 3600) THEN 'due'
                                                         ELSE 'later'
                                                         END
            ...
            ORDER BY CASE section WHEN 'due' THEN 1 WHEN 'later' THEN 2 ELSE 3 END,
                animal_name
        `;
  }
  ...
  private buildComplianceQuery() {
    return sql`
            SELECT r.id                                                        as regimen_id,
                   ...
                   COUNT(*)                                                       FILTER (WHERE a.status = 'ON_TIME') as on_time, COUNT(*) FILTER (WHERE a.status IN ('LATE', 'VERY_LATE')) as late, COUNT(*) FILTER (WHERE a.status = 'MISSED') as missed, ROUND(
