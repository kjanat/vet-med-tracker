	...
	const materializedQuery = async () => {
		return db.execute(sql`
			SELECT 
				...
				animal_id,
				day,
				SUM(total_doses) as total,
				SUM(on_time_count) as on_time,
				SUM(late_count + very_late_count) as late,
	...
	const materializedQuery = async () => {
		return db.execute(sql`
			SELECT
				...
				medication_id,
				month,
				SUM(total_administrations) as total_administrations,
				AVG(animals_treated) as animals_treated,
...
async function testViewHealth(): Promise<TestResult> {
	...
	try {
		const healthResult = await db.execute(
			sql`SELECT * FROM get_mv_refresh_status()`,
		);
